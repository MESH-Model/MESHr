...
#' @param IDW_file Required. Output file which holds gridded precipitation for all
#' time steps. 
...
#' is gridded. This saves on memory, but can be quite slow. 
#' @export
#'
#' @examples \dontrun{
...
gridPrecip <- function(precip = NULL, shed_raster = NULL, 
  ...
  xdelta <- paste(":xDelta                   ", raster::res(elev_raster)[1], sep = "")
  ydelta <- paste(":yDelta                   ", raster::res(elev_raster)[2], sep = "")
  ...
  for (i in 1:num_rows) {
    ...
    p <- hydroTSM::hydrokrige(h2, x.gis = site_info, 
                              X = "x", Y = "y", 
    ...
    for (row in 1:rows) {
      output_str <- formatC(t_flipped[row,], digits = 7, width = 10, 
                            format =  "f")
